...
function sendFeedback() {
    fetch('/api/client/send_feedback', {
        ...
        if (datas['result']) {
            alert('Cảm ơn bạn đã gửi phản hồi cho chúng tôi! Phản hồi của bạn sẽ góp phần giúp chúng tôi cải thiện dịch vụ của mình')
            resetFeedbackData()
        }
        else {
            alert('Có một số sự cố xảy ra trong quá trình gửi! Vui lòng thử lại sau!')
        }
